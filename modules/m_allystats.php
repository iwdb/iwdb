...
function getColor($tuedel) {
  ...
  return ("#" . str_pad(dechex($rot), 2, "0", STR_PAD_LEFT) . str_pad(dechex($gruen), 2, "0", STR_PAD_LEFT) . "00");
	...
	if ($row['objekt']=="Kolonie" && $row['typ']=="Eisplanet") $eisis++;
	if ($row['objekt']=="Kolonie" && $row['typ']=="Steinklumpen") ($row['coords_gal'] % 4 == 0) ? $sgsteinis++ : $steinis++;
	if ($row['objekt']=="Kolonie" && $row['typ']=="Gasgigant") $gasis++;
	...
	if ($row['coords_sys']>300) $row['coords_sys']=300;         //dafür sorgen, dass gaaaaanz hinten liegende systeme in das letzte intervall gesteckt werden
