...
if (!empty($coords)) {
	...
	//Spieler gesperrt oder im UMode
	$sql = "SELECT umode, gesperrt FROM " . $db_tb_highscore . " WHERE name = '" . $row['user'] . "'";
	$result = $db->db_query($sql)
        or error(GENERAL_ERROR, 'Could not query config information.', '', __FILE__, __LINE__, $sql);
	$result = $db->db_fetch_array($result);
	$row_umode = $result["umode"];
	$row_gesperrt = $result["gesperrt"];
	
	//Aktualisierung UMode und Gesperrt
	/*
	$aendern = getVar('aendern');
	$sql = "UPDATE " . $db_tb_highscore .
			" SET umode='" . $row_umode .
			"', gesperrt='" . $row_gesperrt .
			"' WHERE name = '" . $row['user'] . "'";
	$result = $db->db_query($sql)
            or error(GENERAL_ERROR, 'Could not query config information.', '', __FILE__, __LINE__, $sql);*/
...
if ((($ansicht == "auto") && ($row['objekt'] != "---")) || ($ansicht == "taktisch") || ($ansicht == "beide")) {
	...
	<tr>
		...
		<td class="windowbg1"><?php echo "<input type='checkbox' name='umode' value='0'";
		 if ($row_umode == "1") {
	...
	<tr>
		...
		<td class="windowbg1"><?php echo "<input type='checkbox' name='gesperrt' value='0'";
		 if ($row_gesperrt == "1") {
			echo 'checked="checked"';
		 }
		 ?>
	...
	<tr>
		...
		<td class='windowbg1' align='center'>
                    <input type='submit' name='aendern' value='Status ändern' class='submit'>
		</td>
	</tr>
<?php
}
