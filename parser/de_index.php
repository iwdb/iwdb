...
{
    global $db, $db_tb_lieferung, $db_tb_scans, $db_tb_incomings, $config_date;
    ...
    if (($scan_data['art'] == "Angriff") || (($scan_data['art'] == "Sondierung (Schiffe/Def/Ress)") || ($scan_data['art'] == "Sondierung (Gebäude/Ress)"))) {
        ...
        if (!empty($db_tb_incomings)) {
            if ($scan_data['time'] > ($config_date - 20 * 60)) {

                $sql = "DELETE FROM $db_tb_incomings WHERE timestamp<" . ($config_date - 20 * 60);
                $result = $db->db_query($sql)
                    or error(GENERAL_ERROR, 'Could not query config information.', '', __FILE__, __LINE__, $sql);
