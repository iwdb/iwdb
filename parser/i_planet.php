...
function updateplanet()
{
    ...
    if ((trim($row['objekt']) != 'Kampfbasis') AND ((trim($row['typ']) <> trim($scan_data['typ'])) OR ((trim($row['user']) != trim($scan_data['user'])) && !empty($row['user'])))) {
        ...
        if (!empty($db_tb_scans_historie)) {
            $sql = "DELETE FROM " . $db_tb_scans_historie . " WHERE coords='" . $scan_data['coords'] . "'";
            $result = $db->db_query($sql)
                or error(GENERAL_ERROR, 'Could not query config information.', '', __FILE__, __LINE__, $sql);
        }
        ...
        $sql = "INSERT INTO " . $db_tb_scans . " (" . $sql_key . ") VALUES (" . $sql_data . ")";
        $result = $db->db_query($sql)
            or error(GENERAL_ERROR, 'Could not query config information.', '', __FILE__, __LINE__, $sql);
        ...
        if (($row['user'] == '') AND ($row['user'] != $scan_data['user'])) {
            $update = "reserviert=''";
        }
        ...
        $sql = "UPDATE " . $db_tb_scans . " SET " . $update . " WHERE coords='" . $scan_data['coords'] . "'";
        $result = $db->db_query($sql)
            or error(GENERAL_ERROR, 'Could not query config information.', '', __FILE__, __LINE__, $sql);
