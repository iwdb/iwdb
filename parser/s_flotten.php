...
function parse_flotten($scanlines, $eigene = true) {
	...
	$state = 0;
	$arts = 'Angriff|Transport|Ressourcen\sabholen|Übergabe\s\(tr\sSchiffe\)|Übergabe|Rückkehr|Stationieren|Sondierung|Ressourcenhandel\s\(ok\)|Ressourcenhandel|Safeflug|Kolonisierung';
	$replaces = array(")" => "\)", "(" => "\(");
	...
	foreach($scanlines as $scan) {
		if (str_replace(' ', '', $scan) == '')
			continue;
		if ($scan == "Werft- / Schiffbauübersicht" ||
		    $scan == "Ausbaustatus" ||
		...
		case 3:
			...
			break;
		// Noch ein state f�r fremde flotten
		case 4:
			debug_var('state', $state = 2);
			break;
		}
