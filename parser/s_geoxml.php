...
/*
  ALTER TABLE `prefix_scans` ADD `reset_timestamp` INT( 11 ) NOT NULL DEFAULT '0';
*/
	...
	foreach ($scanlines as $line) {
		if (strpos($line, "http://www.icewars.de/portal/kb/de/sb.php") !== FALSE) {
			...
			foreach ($ressourcen as $ressource) {
				...
				case 1: $scan_data["eisengehalt"] = $wert;
				case 4: $scan_data["eisdichte"] = $wert;
				case 5: $scan_data["chemievorkommen"] = $wert;
				}
			}
			...
			foreach ($ressourcen_tech_team as $ressource_tech_team) {
				...
				case 1: $scan_data["tteisen"] = $wert;
				case 4: $scan_data["tteis"] = $wert;
				case 5: $scan_data["ttchemie"] = $wert;
				}
			}
			...
			foreach ($xml->plani_data->besonderheiten->besonderheit as $besonderheit) {
				if (isset($scan_data['besonderheiten']))
					$scan_data['besonderheiten'] .= ", " . str_replace( "Ã¼", "ü", (string)$besonderheit->name);
				else
					$scan_data['besonderheiten'] = str_replace( "Ã¼", "ü", (string)$besonderheit->name);
