...
//*****************************************************************************
//
// Ressie-�bersicht parsen: Suche Klammerpaare und ordne diese der in der
...
function parse_production($scanlines) {
 ...
 $scanwhole = implode("\n",$scanlines);
			  ...
			  echo "<div class='system_notification'>Produktion Teil 2 aktualisiert/hinzugefügt mit den Werten:</div>";
  ...
  foreach($scanlines as $scan) {
	...
	// Das Gesamt-Paket war nicht enthalten? Kommentarlos zur�ckschicken.
	if($foundGesamt === FALSE)
	  return;
	
  // Wir haben keinen vollstaendigen Bericht, wenn nicht alle 7 Zeilen gesetzt sind.
  if($line != 7) {
    echo "<div class='system_error'>Produktionsbericht unvollständig.</div>";
    return;    
  }
