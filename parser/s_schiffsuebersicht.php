...
function parse_schiffsuebersicht($scanlines) {
    ...
    // die Basen in den ersten Zeilen aufgef�hrt. Erst wenn die aktuelle 
    // Zeile "Flug", "Stat" und "Gesamt" enth�lt, sind in den folgenden
    ...
    if ( $start === 3 )
    {
      ...
      if( !empty($temp[0])) {
        ...
        $schiffsname = str_replace(" ","%", $schiffsname);     
                ...
                // Massdriver Pakete sind keine g�ltigen Schiffe. 
              	if( $schiffsname !== "Massdriver%Paket" ) {
                    ...
                    // ID nicht gefunden -> neu einf�gen.
                		if( empty($row['id'])) {
/*
        $schiffsname = str_replace("%%","%", $schiffsname);
        $schiffsname = str_replace("%"," ", $schiffsname); 

        echo "<div class='doc_red'>Neues Schiff wurde hinzugefügt:<br><pre>";
        print("[".$schiffsname."]");
        echo "</pre></div> ";   
	...
	$deleted = true;
                    ...
                    // Letzte Zahl der Zeile ist die Gesamtzahl der Schiffe f�r 
                    // diesen Schiffstyp.
            ...
            // Noch im Prolog. Pr�fen, ob der Prolog jetzt abgeschlossen ist.
      if( strpos($scan, "Flug") > 0 && strpos($scan, "Gesamt") > 0 && strpos($scan, "Stat") > 0)
      {
        $start = 3;
        $lines = $lines + 2;
        } else {
        if ( strpos($scan, "Schiffsübersicht") === 0 ) {
          $start = 2;
        }
        if ( $start == 2 ) $lines++;
      }
